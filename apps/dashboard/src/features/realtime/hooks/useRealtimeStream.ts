...
interface UseRealtimeStreamOptions {
  ...
  /** 帧新鲜度阈值（毫秒）：everSucceeded 后超时无新帧 → interrupted 并启动 REST 兜底（默认 3 × 推送周期 5s） */
  frameStaleTimeoutMs?: number;
...
/**
 * 帧新鲜度阈值：15_000ms = 3 × 服务端推送周期（5s）。
 * 取值依据：容忍连续丢失 2 帧不误报（网络抖动/单帧延迟），第 3 个周期仍无帧即判中断；
 * 断流最迟 15s 翻转 interrupted（满足 AC-M02-007 复测「≤30s 出现更新失败横幅」），
 * 恢复后由 5s 兜底轮询/EventSource 自动重链接管（满足「≤15s 横幅消失」）。
 */
export const FRAME_STALE_TIMEOUT_MS = 15_000;
 ...
 *   - 断流兜底：经代理的半开连接不会触发 error 事件，由「帧新鲜度看门狗」判定
 *     （everSucceeded 后超 FRAME_STALE_TIMEOUT_MS 无任何新帧 → interrupted 并启动 REST 兜底轮询；
 *     此后任何新帧到达（SSE 重连或兜底轮询成功）→ 回 live）
 * - idle：未选定应用
 *
...
export function useRealtimeStream(
  ...
  const {
    ...
    frameStaleTimeoutMs = FRAME_STALE_TIMEOUT_MS,
  ...
  useEffect(() => {
    ...
    let frameWatchdogTimer: number | undefined;
    let source: EventSource | undefined;

    /** 统一清理：所有计时器（含兜底轮询与帧看门狗）必须在切换应用/卸载时清空，防泄漏 */
    const timersClear = (): void => {
      ...
      if (frameWatchdogTimer !== undefined) {
        window.clearTimeout(frameWatchdogTimer);
        frameWatchdogTimer = undefined;
      }
    };

    /** 帧新鲜度看门狗：每收到一帧重新武装；超时无新帧（半开连接无 error 事件）→ interrupted + 兜底轮询 */
    const armFrameWatchdog = (): void => {
      if (frameWatchdogTimer !== undefined) window.clearTimeout(frameWatchdogTimer);
      frameWatchdogTimer = window.setTimeout(() => {
        frameWatchdogTimer = undefined;
        if (disposed || !everSucceededRef.current) return;
        // 与 error 路径语义一致：曾成功后超时 → interrupted（保留旧快照），REST 兜底接管
